...
(
    ...
    `attributes`      VARCHAR(255) COMMENT 'Additional terms to describe this alternative title, not enumerated',
...
(
    ...
    `parentTconst`  VARCHAR(100) COMMENT 'The alphanumeric identifier of the parent title.',
...
(
    ...
    `titleType`      VARCHAR(20)  COMMENT 'The type/format of the title (e.g. movie, short, tvseries, tvepisode, video, etc).',
    `primaryTitle`   VARCHAR(100) COMMENT 'The more popular title / the title used by the filmmakers on promotional materials at the point of release.',
    `originalTitle`  VARCHAR(100) COMMENT 'Original title, in the original language.',
    `isAdult`        VARCHAR(2)   COMMENT '0: non-adult title; 1: adult title.',
    `startYear`      TINYINT            DEFAULT '-1' COMMENT 'Represents the release year of a title. In the case of TV Series, it is the series start year.',
    `endYear`        TINYINT      DEFAULT '0' COMMENT 'TV Series end year. ‘0’ for all other title types.',
    `runtimeMinutes` INT           DEFAULT '-1' COMMENT 'Primary runtime of the title, in minutes.',
...
(
    `tconst`     VARCHAR(12) NOT NULL COMMENT 'The alphanumeric identifier of the title.',
    `directors`  VARCHAR(48) COMMENT 'A comma separated list of nconsts of directors.',
    `writers`    VARCHAR(48) COMMENT 'A comma separated list of nconsts of writers.',
...
(
    ...
    `nconst`     VARCHAR(12)  COMMENT 'The alphanumeric identifier of the name/person.',
    `category`   VARCHAR(100) COMMENT 'The category of job that person was in.',
    `job`        VARCHAR(100) COMMENT 'The specific job title if applicable, else '\N'.',
    `characters` VARCHAR(100) COMMENT 'The name of the character played if applicable, else '\N'.',
    `update_time` DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '更新时间'
) ENGINE = OLAP
...
(
    ...
    `primaryName`       VARCHAR(100) COMMENT 'The name by which the person is most often credited.',
    ...
    `primaryProfession` VARCHAR(100) COMMENT 'The top-3 professions of the person.',
    `knownForTitles`    VARCHAR(100) COMMENT 'Titles the person is known for.',
    `update_time`       DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '更新时间'
) ENGINE = OLAP
